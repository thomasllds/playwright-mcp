...
async function installHttpTransport(httpServer, serverBackendFactory, allowedHosts) {
  ...
  const allowedHostnames = new Set(allowedHosts.map((entry) => entry.split(":")[0]));
  ...
  httpServer.on("request", async (req, res) => {
    if (!allowAnyHost) {
      ...
      const hasAllowedHost = allowedHosts.includes(hostValue) || allowedHosts.includes(hostName) || allowedHostnames.has(hostValue) || allowedHostnames.has(hostName);
      if (!hasAllowedHost) {
        res.statusCode = 403;
