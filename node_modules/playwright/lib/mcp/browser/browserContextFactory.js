...
class CdpContextFactory extends BaseContextFactory {
  ...
  async _doObtainBrowser() {
    const endpoint = this.config.browser.cdpEndpoint;
    const timeout = this.config.browser.cdpTimeout ?? 3e4;
    const retries = this.config.browser.cdpRetries ?? 5;
    ...
    for (let attempt = 1; attempt <= retries; attempt++) {
      try {
        ...
        console.error(`Failed to connect to CDP endpoint (attempt ${attempt}/${retries}): ${redactedEndpoint}`);
        if (attempt === retries)
          throw error;
        await sleep(Math.min(1e3 * attempt, 5e3));
      }
    }
    throw new Error(`Failed to connect to CDP endpoint: ${redactedEndpoint}`);
  }
