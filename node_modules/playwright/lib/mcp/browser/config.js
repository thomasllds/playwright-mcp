...
function configFromEnv() {
  const options = {};
  const shouldDefaultServer = Boolean(process.env.PLAYWRIGHT_MCP_SERVER || process.env.PLAYWRIGHT_MCP_HOST || process.env.PLAYWRIGHT_MCP_PORT || process.env.PORT || process.env.PLAYWRIGHT_MCP_ALLOWED_HOSTS || process.env.PLAYWRIGHT_MCP_ALLOWED_HOSTNAMES || process.env.PLAYWRIGHT_MCP_CDP_ENDPOINT);
  const allowedHostsValue = process.env.PLAYWRIGHT_MCP_ALLOWED_HOSTS ?? process.env.PLAYWRIGHT_MCP_ALLOWED_HOSTNAMES;
  options.allowedHosts = parseListFromEnv(allowedHostsValue ?? (shouldDefaultServer ? "*" : void 0), { allowWildcard: true });
  options.allowedOrigins = parseListFromEnv(process.env.PLAYWRIGHT_MCP_ALLOWED_ORIGINS ?? (shouldDefaultServer ? "*" : void 0));
  options.allowUnrestrictedFileAccess = envToBoolean(process.env.PLAYWRIGHT_MCP_ALLOW_UNRESTRICTED_FILE_ACCESS);
  options.blockedOrigins = parseListFromEnv(process.env.PLAYWRIGHT_MCP_BLOCKED_ORIGINS);
  options.blockServiceWorkers = envToBoolean(process.env.PLAYWRIGHT_MCP_BLOCK_SERVICE_WORKERS);
  options.browser = envToString(process.env.PLAYWRIGHT_MCP_BROWSER);
  options.caps = parseListFromEnv(process.env.PLAYWRIGHT_MCP_CAPS);
  options.cdpEndpoint = envToString(process.env.PLAYWRIGHT_MCP_CDP_ENDPOINT);
  options.cdpHeader = parseCdpHeadersEnv(process.env.PLAYWRIGHT_MCP_CDP_HEADERS);
  ...
  options.grantPermissions = parseListFromEnv(process.env.PLAYWRIGHT_MCP_GRANT_PERMISSIONS);
  options.headless = envToBoolean(process.env.PLAYWRIGHT_MCP_HEADLESS);
  options.host = envToString(process.env.PLAYWRIGHT_MCP_HOST ?? (shouldDefaultServer ? "0.0.0.0" : void 0));
  ...
  options.port = numberParser(process.env.PLAYWRIGHT_MCP_PORT ?? process.env.PORT ?? (shouldDefaultServer ? "8931" : void 0));
  ...
  options.sharedBrowserContext = envToBoolean(process.env.PLAYWRIGHT_MCP_SHARED_BROWSER_CONTEXT);
...
function headerParser(arg, previous) {
  ...
  if (!name || value === void 0 || value === "")
    return result;
  result[name] = value;
  return result;
}
function parseCdpHeadersEnv(value) {
  if (!value)
    return void 0;
  ...
  const headers = {};
  for (const entry of parseListFromEnv(trimmed) || []) {
    if (!entry)
      continue;
    headerParser(entry, headers);
  }
...
function parseListFromEnv(value, options) {
  if (value === void 0)
    return void 0;
  const raw = String(value).trim();
  if (!raw)
    return void 0;
  if (options?.allowWildcard && raw === "*")
    return ["*"];
  if (raw === "*")
    return void 0;
  const result = [];
