...
function redactEndpoint(endpoint) {
  ...
  try {
    ...
    return endpoint.replace(/(token|access_token|auth|key)=([^&]+)/gi, "$1=REDACTED");
  }
}
function logEffectiveConfig(config) {
  const serverConfig = config.server || {};
  const allowedHosts = serverConfig.allowedHosts?.length ? serverConfig.allowedHosts.join(", ") : "default";
  const allowedOrigins = config.network?.allowedOrigins?.length ? config.network.allowedOrigins.join("; ") : "default";
  const sharedContext = config.sharedBrowserContext ?? false;
  const cdpEndpoint = redactEndpoint(config.browser?.cdpEndpoint);
  const lines = [
    "Playwright MCP config:",
    `  host: ${serverConfig.host ?? "default"}`,
    `  port: ${serverConfig.port ?? "stdio"}`,
    `  allowedHosts: ${allowedHosts}`,
    `  allowedOrigins: ${allowedOrigins}`,
    `  sharedBrowserContext: ${sharedContext}`,
    `  cdpEndpoint: ${cdpEndpoint ?? "none"}`
  ];
